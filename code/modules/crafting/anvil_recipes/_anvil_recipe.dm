			...
			if(amt2raise > 0)
				if(!HAS_TRAIT(user, TRAIT_MALUMFIRE))
					...
					if(skill_level < 3) // Non-blacksmith jobs can't level past 3. Ever.
						user.adjust_experience(appro_skill, floor(amt2raise * boon), FALSE) // STONEKEEP ORIGINAL CODE //Stonekeep Edit
				else
					...
					if(skill_level < 3)
						...
						user.adjust_experience(appro_skill, amt2raise * boon, FALSE) // STONEKEEP ORIGINAL CODE //Stonekeep Edit
						...
						user.adjust_experience(appro_skill, amt2raise, FALSE) // STONEKEEP ORIGINAL CODE //Stonekeep Edit
